...
export class ChannelStartupService {
  ...
  public async fetchChats(query: any) {
    ...
    const searchFilter = query?.where?.pushName?.contains
      ? Prisma.sql`AND (
          C.pushName LIKE ${`%${query.where.pushName.contains}%`} OR 
          M.pushName LIKE ${`%${query.where.pushName.contains}%`} OR 
          JSON_UNQUOTE(JSON_EXTRACT(M.\`key\`, '$.remoteJid')) LIKE ${`%${query.where.pushName.contains}%`}
        )`
      : query?.where?.remoteJid?.contains
        ? Prisma.sql`AND JSON_UNQUOTE(JSON_EXTRACT(M.\`key\`, '$.remoteJid')) LIKE ${`%${query.where.remoteJid.contains}%`}`
        : Prisma.sql``;

    const timestampFilter =
      query?.where?.messageTimestamp?.gte && query?.where?.messageTimestamp?.lte
        ? Prisma.sql`
        AND M.messageTimestamp >= ${Math.floor(new Date(query.where.messageTimestamp.gte).getTime() / 1000)}
        AND M.messageTimestamp <= ${Math.floor(new Date(query.where.messageTimestamp.lte).getTime() / 1000)}`
        : Prisma.sql``;
    ...
    const results = await this.prismaRepository.$queryRaw`
      WITH rankedMessages AS (
        SELECT 
          C.id as contactId,
          JSON_UNQUOTE(JSON_EXTRACT(M.\`key\`, '$.remoteJid')) as remoteJid,
          CASE 
            WHEN JSON_UNQUOTE(JSON_EXTRACT(M.\`key\`, '$.remoteJid')) LIKE '%@g.us' THEN COALESCE(CH.name, C.pushName)
            ELSE COALESCE(C.pushName, M.pushName)
          END as pushName,
          C.profilePicUrl,
          COALESCE(
            FROM_UNIXTIME(M.messageTimestamp), 
            C.updatedAt
          ) as updatedAt,
          CH.name as chatName,
          CH.createdAt as windowStart,
          DATE_ADD(CH.createdAt, INTERVAL 24 HOUR) as windowExpires,
          CH.unreadMessages as unreadMessages,
          CASE WHEN DATE_ADD(CH.createdAt, INTERVAL 24 HOUR) > NOW() THEN true ELSE false END as windowActive,
          M.id AS lastMessageId,
          M.key AS lastMessage_key,
          CASE
            WHEN JSON_EXTRACT(M.\`key\`, '$.fromMe') = true THEN 'Você'
            ELSE M.pushName
          ...
          ROW_NUMBER() OVER(PARTITION BY JSON_EXTRACT(M.\`key\`, '$.remoteJid') ORDER BY M.messageTimestamp DESC) as rn
        FROM Message M
        LEFT JOIN Contact C ON C.remoteJid = JSON_UNQUOTE(JSON_EXTRACT(M.\`key\`, '$.remoteJid')) AND C.instanceId = M.instanceId
        LEFT JOIN Chat CH ON CH.remoteJid = JSON_UNQUOTE(JSON_EXTRACT(M.\`key\`, '$.remoteJid')) AND CH.instanceId = M.instanceId
        WHERE M.instanceId = ${this.instanceId}
        ${remoteJid ? Prisma.sql`AND JSON_UNQUOTE(JSON_EXTRACT(M.\`key\`, '$.remoteJid')) = ${remoteJid}` : Prisma.sql``}
        ${searchFilter}
        ${timestampFilter}
      )
