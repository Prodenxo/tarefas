...
export class BaileysStartupService extends ChannelStartupService {
  ...
  private async updateChatUnreadMessages(remoteJid: string): Promise<number> {
    const [chat, unreadMessages] = await Promise.all([
      this.prismaRepository.chat.findFirst({ where: { remoteJid, instanceId: this.instanceId } }),
      // Use raw SQL to avoid JSON path issues
  ...
  private async addLabel(labelId: string, instanceId: string, chatId: string) {
    ...
    await this.prismaRepository.$executeRawUnsafe(
      `INSERT INTO Chat (id, instanceId, remoteJid, labels, createdAt, updatedAt)
       VALUES (?, ?, ?, JSON_ARRAY(?), NOW(), NOW()) ON DUPLICATE KEY UPDATE
          labels = JSON_ARRAY_APPEND(IFNULL(labels, JSON_ARRAY()), '$', ?),
          updatedAt = NOW();`,
      id,
      instanceId,
      chatId,
  ...
  private async removeLabel(labelId: string, instanceId: string, chatId: string) {
    ...
    await this.prismaRepository.$executeRawUnsafe(
      `INSERT INTO Chat (id, instanceId, remoteJid, labels, createdAt, updatedAt)
       VALUES (?, ?, ?, JSON_ARRAY(), NOW(), NOW()) ON DUPLICATE KEY UPDATE
          labels = IF(JSON_SEARCH(labels, 'one', ?) IS NOT NULL, 
                      JSON_REMOVE(labels, JSON_UNQUOTE(JSON_SEARCH(labels, 'one', ?))), 
                      labels),
          updatedAt = NOW();`,
      id,
      instanceId,
